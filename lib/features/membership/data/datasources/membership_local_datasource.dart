...
abstract class MembershipLocalDataSource {
  ...
  Future<MembershipApplicationModel?> getLastApplication();
}

class MembershipLocalDataSourceImpl implements MembershipLocalDataSource {
  final SharedPreferences sharedPreferences;

  MembershipLocalDataSourceImpl(this.sharedPreferences);

  @override
  Future<void> cacheApplication(MembershipApplicationModel application) async {
    await sharedPreferences.setString(
      'CACHED_APPLICATION',
      jsonEncode(application.toMap()),
    );
  }

  @override
  Future<MembershipApplicationModel?> getLastApplication() async {
    final jsonString = sharedPreferences.getString('CACHED_APPLICATION');
    if (jsonString != null) {
      // You'll need to implement fromMap in the model
